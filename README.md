...
- 只有精确命中的映射才会改写模型；没有映射或映射未命中时，Codex 选择的模型原样透传。
- 供应商配置中的单值模型是启动默认值，不会覆盖当前请求的显式模型。
...
- 只有精确命中的映射才会改写模型；没有映射或映射未命中时，模型名都**原样透传**给上游。
- 供应商编辑页的"启动默认模型"保留配置中的默认值，但不会覆盖 Codex 当前请求的显式模型。
...
如果报错是“模型名不被上游接受”（例如 `The supported API model names are ... but you passed ...`），说明客户端发送的模型名与当前供应商不匹配。请为该供应商配置明确的**模型映射**（本地模型名 → 上游模型名）；未命中的模型会原样透传。供应商配置中的启动默认模型不会隐式改写当前请求。
