...
const THEME_STORAGE_KEY = "claude-local-proxy-theme";
...
function renderRuntimeSettings(settings) {
  ...
  runtimePortInput.value = String(settings.configured_port ?? settings.active_port ?? 17891);
  ...
  document.querySelector("#runtime-data-directory").textContent = settings.data_directory || "~/.claude-local-proxy";
  document.querySelector("#runtime-codex-config").textContent = settings.claude_config_file || "环境变量";
...
function renderProviderList() {
  ...
  for (const provider of providers) {
    const row = document.createElement("div");
    row.className = `provider-row${provider.current ? " current" : ""}${provider.hidden ? " hidden-provider" : ""}${provider.compatible ? "" : " incompatible-provider"}${manageProvidersMode ? " managing" : ""}`;
    ...
    state.append(provider.hidden ? "已隐藏" : !provider.compatible ? "协议不兼容" : provider.current ? "当前使用" : "可切换");

    const copy = document.createElement("button");
    copy.type = "button";
    copy.className = "provider-copy";
    copy.disabled = manageProvidersMode || !provider.compatible || !provider.has_credentials;
    copy.setAttribute(
      "aria-label",
    ...
    copy.addEventListener("click", (event) => {
      event.stopPropagation();
      if (!manageProvidersMode && provider.compatible && provider.has_credentials) selectProvider(provider);
    });
    row.addEventListener("click", (event) => {
      if (
        ...
        if (provider.compatible && provider.has_credentials) selectProvider(provider);
      }
    });
  ...
  document.querySelector("#provider-count").textContent = query
    ...
    : manageProvidersMode
      ...
      : `已显示 ${visibleCount} 个 Claude Code 供应商`;
}
...
function renderStatus(status) {
  ...
  document.querySelector("#wire-api").textContent = current?.wire_api === "anthropic_messages" ? "Messages · SSE" : escapeText(current?.wire_api || "—");
...
async function validateRuntimeDatabase() {
  ...
  try {
    ...
    showToast(
      "数据来源可用",
      `已读取 ${result.provider_count} 个 Claude Code 供应商。`,
    );
  } catch (error) {
...
async function copyConfig() {
  try {
    const response = await fetch("/control/api/claude-config", { cache: "no-store" });
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    const snippets = JSON.parse(await response.text());
    await navigator.clipboard.writeText(snippets.powershell || snippets.bash || "");
    showToast("Claude Code 配置已复制", "在当前终端运行配置后启动 Claude Code。");
  } catch (error) {
    showToast("复制失败", "浏览器没有允许写入剪贴板。", "error");
  }
}
...
async function shutdownProxy() {
  if (!window.confirm("退出本地中转后，Claude Code 新请求将无法连接。确定退出吗？")) return;
  try {
