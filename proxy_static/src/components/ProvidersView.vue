...
function recoveryItemKey(item) { return item.event_count != null ? ['grouped', item.request_id, item.request_started_at ?? ''].join('-') : [item.recorded_at, item.request_id, item.provider_id, item.attempt, item.outcome].join('-') }
function recoveryMeta(item) { const provider = providers.value.find(value => value.provider_id === item.provider_id); const kind = ({ rate_limited: '频率限制', model_capacity: '模型容量', connection: '连接失败', stream_start: '流启动失败', stream_idle_timeout: '流空闲超时' })[item.kind] || item.kind || '临时故障'; const outcome = ({ retrying: '重试中', exhausted: '重试结束', client_disconnected: '已取消', passed_through: '已透传' })[item.outcome] || '已记录'; const eventCount = Number(item.event_count); return [outcome, `最后 ${formatHistoryTime(item.recorded_at)}`, item.request_started_at ? `开始 ${formatHistoryTime(item.request_started_at)}` : null, provider?.name || item.provider_id || '未知供应商', `第 ${item.attempt || 1} 次尝试`, Number.isFinite(eventCount) && eventCount > 1 ? `${eventCount} 次错误事件` : null, kind].filter(Boolean).join(' · ') }
