<template>
  <section id="runtime-view" class="settings-view view-panel" aria-labelledby="runtime-title">
    ...
    <form class="settings-form" @submit.prevent="saveSettings">
      ...
      <label v-if="config.features?.status_upload" class="setting-row setting-toggle"><span><strong>供应商上传检测</strong><small>在供应商卡片中显示“上传检测”按钮</small></span><input v-model="settings.show_status_upload" type="checkbox" /></label>
...
const settings = ref({ configured_port: 17890, active_port: 17890, database_path: '', health_status_url: '', show_provider_launch_command: true, show_status_upload: true })
const update = ref({})
