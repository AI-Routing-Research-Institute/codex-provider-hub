...
async function loadSettings() { try { settings.value = { ...settings.value, ...(await controlFetch('/api/runtime-settings')) }; settings.value.health_status_url ||= '' } catch (error) { message.value = `读取失败：${error.message}` }; try { update.value = await controlFetch('/api/update') } catch {} }
...
async function saveSettings() { saving.value = true; message.value = ''; try { settings.value = await controlFetch('/api/runtime-settings', jsonOptions('POST', { port: Number(settings.value.configured_port), database_path: settings.value.database_path, health_status_url: settings.value.health_status_url || null, ...(props.config.features?.provider_launch_command ? { show_provider_launch_command: settings.value.show_provider_launch_command } : {}) })); settings.value.health_status_url ||= ''; message.value = settings.value.restart_required ? '设置已保存，重启后使用新端口。' : '运行设置已保存。' } catch (error) { message.value = `保存失败：${error.message}` } finally { saving.value = false } }
