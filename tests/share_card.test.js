...
test("terminalCardTheme pins the terminal card palette, fonts, and metrics", async () => {
  ...
  assert.equal(theme.cardWidth, 360)
  assert.equal(theme.cardHeight, 640)
  assert.equal(theme.cornerRadius, 16)
  assert.equal(theme.titlebarHeight, 36)
  assert.equal(theme.bodyPaddingX, 24)
  assert.ok(!("margin" in theme), "full-bleed export must not reserve outer margin")
